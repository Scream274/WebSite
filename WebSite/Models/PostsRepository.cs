...
namespace WebSite.Models
{
    public class PostsRepository
    {
        ...
        internal IQueryable<Post> GetPostsByCategory(string categorySlug)
        {
            ...
            return (from p in _dbContext.Posts join c in _dbContext.Categories on p.CategoryId equals c.Id where p.PostStatus == Status.CRERATED && c.Slug == categorySlug select p);
        }
        ...
        internal IQueryable<Post> GetPostsByTags(string tagSlug)
        {
            return (from p in _dbContext.Posts
                    ...
                    where p.PostStatus == Status.CRERATED && t.Name == tagSlug
                    select p);
        }
